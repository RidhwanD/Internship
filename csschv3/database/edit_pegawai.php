...
try{
	$sql = "UPDATE pegawai SET namaPegawai ='$nama', kontak='$kontak', jenisKelamin='$jkP' WHERE idPegawai= '$get_id' ";
	...
	if (isset($_POST['shift1'])) {
		...
		$sql = "INSERT INTO shiftPegawai VALUES (".$get_id.",'".$sft."','".$namaA."')";
		$conn->exec($sql);
	}
	...
	if (isset($_POST['shift3'])) {
		...
		if (isset($_POST['area_shift3_1'])and isset($_POST['area_shift3_2'])){
			...
			if ($area31 == $area32) {
				...
				$sql = "INSERT INTO shiftPegawai VALUES (".$get_id.",'".$sft."','".$namaA."')";
				$conn->exec($sql);
				$sql = "INSERT INTO shiftPegawai VALUES (".$get_id.",'".$sft."','".$namaAS."')";
				$conn->exec($sql);
			} 
		} else {
	...
	echo "<script>alert('Data Pegawai Berhasil Diubah!'); window.location='../pages/daftar_pegawai.php'</script>";
} catch (Exception $e){
